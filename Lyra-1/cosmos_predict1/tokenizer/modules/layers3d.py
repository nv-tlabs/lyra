...
class DecoderFactorizedVanilla(nn.Module):
    def __init__(
        self,
        out_channels: int,
        channels: int,
        ...
        spatial_compression: int = 16,
        temporal_compression: int = 8,
        do_temporal_up_with_spatial: bool = True,
        always_temporal_up: bool = False,
        temporal_up_all: bool = False,
        **ignore_kwargs,
    ):
        super().__init__()
        self.num_resolutions = len(channels_mult)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            up.block = block
            up.attn = attn
            if i_level != 0 or temporal_up_all:
                # The layer index for temporal/spatial downsampling performed
                # in the encoder should correspond to the layer index in
                # reverse order where upsampling is performed in the decoder.
                # If you've a pre-trained model, you can simply finetune.
                i_level_reverse = self.num_resolutions - i_level - 1
                if always_temporal_up or temporal_up_all:
                    temporal_up = True
                else:
                    if legacy_mode:
                        temporal_up = i_level_reverse < self.num_temporal_ups
                        ...
                        temporal_up = 0 < i_level_reverse < self.num_temporal_ups + 1
                spatial_up = (
                    temporal_up and do_temporal_up_with_spatial
                    ) or (
                        not do_temporal_up_with_spatial and i_level_reverse < self.num_spatial_ups
                        ) or (
                        i_level_reverse < self.num_spatial_ups and self.num_spatial_ups > self.num_temporal_ups
                        )
                up.upsample = CausalHybridUpsample3d(block_in, spatial_up=spatial_up, temporal_up=temporal_up)
    ...
    def forward(self, h, gradient_checkpoint=False):
        h = forward_checkpointing(self.conv_in, h, gradient_checkpoint=gradient_checkpoint)
        # middle block
        h = forward_checkpointing(self.mid.block_1, h, gradient_checkpoint=gradient_checkpoint)
        h = forward_checkpointing(self.mid.attn_1, h, gradient_checkpoint=gradient_checkpoint)
        h = forward_checkpointing(self.mid.block_2, h, gradient_checkpoint=gradient_checkpoint)

        # decoder blocks
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                h = forward_checkpointing(self.up[i_level].block[i_block], h, gradient_checkpoint=gradient_checkpoint)
                if len(self.up[i_level].attn) > 0:
                    h = forward_checkpointing(self.up[i_level].attn[i_block], h, gradient_checkpoint=gradient_checkpoint)
            if i_level != 0:
                h = forward_checkpointing(self.up[i_level].upsample, h, gradient_checkpoint=gradient_checkpoint)
        h = forward_checkpointing(self.norm_out, h, gradient_checkpoint=gradient_checkpoint)
        h = forward_checkpointing(nonlinearity, h, gradient_checkpoint=gradient_checkpoint)
        h = forward_checkpointing(self.conv_out, h, gradient_checkpoint=gradient_checkpoint)

        return h

class DecoderBaseVanilla(nn.Module):
    def __init__(
        self,
        out_channels: int,
        channels: int,
        ...
        **ignore_kwargs,
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            for _ in range(self.num_res_blocks + 1):
                ...
                block_in = block_out
                if curr_res in attn_resolutions and use_causal_attn:
                    attn.append(CausalAttnBlock(block_in, num_groups=num_groups))
            up = nn.Module()
            up.block = block
            up.attn = attn
    ...
    def forward(self, z, gradient_checkpoint=False):
        ...
        # middle block.
        if gradient_checkpoint:
            h = torch.utils.checkpoint.checkpoint(self.mid.block_1, h, use_reentrant=False)
        else:
            h = self.mid.block_1(h)
        if gradient_checkpoint:
            ...
            h = self.mid.attn_1(h)
        if gradient_checkpoint:
            h = torch.utils.checkpoint.checkpoint(self.mid.block_2, h, use_reentrant=False)
        else:
            h = self.mid.block_2(h)
        # decoder blocks.
        for i_level in reversed(range(self.num_resolutions)):
            for i_block in range(self.num_res_blocks + 1):
                if gradient_checkpoint:
                    h = torch.utils.checkpoint.checkpoint(self.up[i_level].block[i_block], h, use_reentrant=False)
                else:
                    h = self.up[i_level].block[i_block](h)
                if len(self.up[i_level].attn) > 0:
                    if gradient_checkpoint:
                        h = torch.utils.checkpoint.checkpoint(self.up[i_level].attn[i_block], h, use_reentrant=False)
                    else:
                        h = self.up[i_level].attn[i_block](h)
            if i_level != 0:
                if gradient_checkpoint:
                    h = torch.utils.checkpoint.checkpoint(self.up[i_level].upsample, h, use_reentrant=False)
                else:
                    h = self.up[i_level].upsample(h)
            else:
                # temporal upsample (last level)
